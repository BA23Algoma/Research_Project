...
fid =  'session_client';
session_client = tcpip_open(fid, '127.0.0.1', 4242);
fopen(session_client);
session_client.Terminator = 'CR/LF';

% Configure data stream
tcpip_write(session_client, '<SET ID="ENABLE_SEND_USER_DATA" STATE="1" />');

% Enable data flow into buffer
tcpip_write(session_client, '<SET ID="ENABLE_SEND_DATA" STATE="1" />');
