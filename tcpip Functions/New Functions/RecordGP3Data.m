...
tcpip_write(session2_client, '<SET ID="ENABLE_SEND_TIME" STATE="1" />');
tcpip_write(session2_client, '<SET ID="ENABLE_SEND_COUNTER" STATE="1" />');
tcpip_write(session2_client, '<SET ID="ENABLE_SEND_USER_DATA" STATE="1" />');

if isempty(varargin) %default
    tcpip_write(session2_client, '<SET ID="ENABLE_SEND_PUPIL_LEFT" STATE="1" />');
    tcpip_write(session2_client, '<SET ID="ENABLE_SEND_PUPIL_RIGHT" STATE="1" />');
    tcpip_write(session2_client, '<SET ID="ENABLE_SEND_EYE_LEFT" STATE="1" />');
    tcpip_write(session2_client, '<SET ID="ENABLE_SEND_EYE_RIGHT" STATE="1" />');
    tcpip_write(session2_client, '<SET ID="ENABLE_SEND_BLINK" STATE="1" />');
elseif ~isempty(varargin)
    for i=1:length(varargin)
        tcpip_write(session2_client, ['<SET ID="' varargin{i} '" STATE="1" />']);
    end
end

% start data server sending data
tcpip_write(session2_client, '<SET ID="ENABLE_SEND_DATA" STATE="1" />');
...
while  1
    ...
    dataReceived = tcpip_read(session2_client, 1024);
...
SendMsgToGP3Updated(session2_client,'CLIENT2_READY');
pause(0.05)
...
tcpip_write(fileID,['DATATYPE\t' repmat('%s\t',1,length(header)) '\n'],header{:});
...
while  session2_client.BytesAvailable > 0
        ...
        if regexp(split{1},'<REC','once')        
            ...
            if strcmp(current_user_data,'STOP_EYETRACKER')
                ...
                tcpip_write(session2_client, '<SET ID="ENABLE_SEND_DATA" STATE="0" />');
