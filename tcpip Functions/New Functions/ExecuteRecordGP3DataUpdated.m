function ExecuteRecordGP3DataUpdated(session1_client, outputFileName, varargin)
...
SendMsgToGP3Updated(session1_client,'');
tcpip_write(session1_client, '<SET ID="ENABLE_SEND_USER_DATA" STATE="1" />');
tcpip_write(session1_client, '<SET ID="ENABLE_SEND_DATA" STATE="1" />');
SendMsgToGP3Updated(session1_client,'START_RECORDING')
...
while  1
    ...
    dataReceived = tcpip_readln(session1_client, 1024);
...
tcpip_write(session1_client, '<SET ID="ENABLE_SEND_DATA" STATE="0" />');
