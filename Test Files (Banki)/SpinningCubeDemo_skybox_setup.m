function SpinningCubeDemo_skybox_setup
...
glMaterialfv(GL_FRONT_AND_BACK,GL_SHININESS,27.8);
 ...
 % Enable cube mapping and generate a cube map texture
glEnable(GL_TEXTURE_CUBE_MAP);
cubeMapTexture = glGenTextures(1);

% Setup textures for all six sides of cube:
for face = 0:5

    % Load the image for the current face using the naming convention
    cubeMapImage = ['cube_map_', num2str(face), '.jpg'];
    imagedata = GlTexture('Textures',cubeMapImage);
    % Set the cube map face target
    target = GL_TEXTURE_CUBE_MAP_POSITIVE_X + face;
    % Enable i'th texture by binding it:
    glBindTexture(GL_TEXTURE_CUBE_MAP,cubeMapTexture);
    % Bind the cube map texture and set parameters
    glBindTexture(GL_TEXTURE_CUBE_MAP, cubeMapTexture);
    glTexImage2D(obj.GL.TEXTURE_2D, 0, obj.GL.RGB, imagedata.nRows, imagedata.nCols, 0, obj.GL.RGB, obj.GL.UNSIGNED_BYTE, imagedata.pixels);
...
while (1)
    ...
    cubeface([ 4 3 2 1 ],cubeMapTexture);
    cubeface([ 5 6 7 8 ],cubeMapTexture);
    cubeface([ 1 2 6 5 ],cubeMapTexture);
    cubeface([ 3 4 8 7 ],cubeMapTexture);
    cubeface([ 2 3 7 6 ],cubeMapTexture);
    cubeface([ 4 1 5 8 ],cubeMapTexture);
    glPopMatrix;
