...
%Load texture
image = GlTexture('Test Files (Banki)', 'initialShadingGroup_Base_Color_Orange.png');

glEnable(GL.TEXTURE_2D);
texture = glGenTextures(1);

% Load texture and set up texture filtering
%image = imread('TeapotTexture.jpg');
...
% texture
%glBindTexture(GL.TEXTURE_2D, modelTexture);
%glGenerateMipmapEXT(GL.TEXTURE_2D);

%Binding Textures
glBindTexture(GL.TEXTURE_2D, texture);
glTexImage2D(GL.TEXTURE_2D, 0, GL.RGB, image.nRows, image.nCols, 0, GL.RGB, GL.UNSIGNED_BYTE, image.pixels);

glTexParameterf(GL.TEXTURE_2D, GL.TEXTURE_MAG_FILTER, GL.LINEAR);
glTexParameterf(GL.TEXTURE_2D, GL.TEXTURE_MIN_FILTER, GL.LINEAR_MIPMAP_LINEAR);
...
glTexParameterf(GL.TEXTURE_2D, GL.TEXTURE_MAX_ANISOTROPY_EXT, maxAnisotropy);
...
while ~KbCheck
    ...
    %glTexCoordPointer(size(obj{1}.texcoords, 1), usetype, 0, obj{1}.texcoords);

    % Adapt startIdx of range: Matlab/Octave use 1-based index into matrices:
   %startIdx = 1;
    %endIdx = size(obj{1}.faces,2) - 1;
    %fcount = endIdx - startIdx + 1;
    
    % Slower rendering path, needed to support OpenGL-1.1 renderers as well:
    %if (size(obj{1}.faces,1)==3)
       % glDrawElements(GL.TRIANGLES, fcount * 3, indextype, obj{1}.faces(:, startIdx:end));
   % elseif size(obj{1}.faces,1)==4
     %   glDrawElements(GL.QUADS, fcount * 4, indextype, obj{1}.faces(:, startIdx:end));
    %else
    ...
    %------Morph Function-------%
     ...
     % Bind the texture and enable vertex and texture coordinate arrays
    %glBindTexture(GL.TEXTURE_2D, modelTexture);
    %glEnableClientState(GL.VERTEX_ARRAY);
    %glEnableClientState(GL.TEXTURE_COORD_ARRAY);

    % Set the vertex and texture coordinate pointers
    %glVertexPointer(3, GL.DOUBLE, 0, obj{1, 1}.vertices);
    %glTexCoordPointer(2, GL.DOUBLE, 0, obj{1, 1}.texcoords);

    % Draw the object with the specified faces
    %glDrawElements(GL.QUADS, numel(obj{1, 1}.faces), GL.UNSIGNED_INT, obj{1, 1}.faces);
