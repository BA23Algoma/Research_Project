...
% Load the 3D model from an OBJ file using Psychtoolbox
objModel = LoadOBJFile('D_Rock_Formation_CM.obj');

% Set the scaling factor and translation for the loaded model
modelScale = 1;  % Adjust this value for the desired scale
modelTranslation = [0, 0, -distanceCm];  % Adjust the translation as needed
...
% load model texture
image = imread(fullfile('D_Rock_Formation_BaseColor.png'));
[s1, s2, s3] = size(image);
...
[texNameFront, targetFront, tuFront, tvFront] = Screen('GetOpenGLTexture', window, modelTexture, imh, imw);
...
while ~KbCheck
    ...
    %glTranslatef(modelTranslation(1), modelTranslation(2), modelTranslation(3));
    ...
    for face = 1:size(objModel{1}.faces, 2)
        for vertex = 1:3
            vertexIndex = objModel{1}.faces(vertex, face) + 1;
            uvIndex = objModel{1}.texcoords(:, vertexIndex);
            glTexCoord2fv(uvIndex);
            glNormal3fv(objModel{1}.normals(:, vertexIndex));
            glVertex3fv(objModel{1}.vertices(:, vertexIndex));
        end
    end
