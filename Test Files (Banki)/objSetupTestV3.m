...
% load object
obj = LoadOBJFile('Conus 2.obj');

% Add one becuase loadOBJFile is zero indexed
obj{1}.faces = obj{1}.faces + 1;
...
% Load texture
jpg = imread('lambert1_Base_Color.png');
[s1, s2, s3] = size(jpg);
...
while ~KbCheck
    ...
    glVertexPointer(size(obj{1}.vertices,1), GL.DOUBLE, 0, obj{1}.vertices);
        ...
        glTexCoordPointer(size(obj{1}.texcoords, 1), GL.DOUBLE, 0, obj{1}.texcoords);

    disp(obj{1}.faces);
    disp(startIdx);
    disp(endIdx);
    if (size(obj{1}.faces,1)==3)
        glDrawElements(GL.TRIANGLES, fcount * 3, GL.UNSIGNED_INT, obj{1}.faces(:, startIdx:endIdx));
    elseif size(obj{1}.faces,1)==4
        glDrawElements(GL.QUADS, fcount * 4, GL.UNSIGNED_INT, obj{1}.faces(:, startIdx:endIdx));
    else
